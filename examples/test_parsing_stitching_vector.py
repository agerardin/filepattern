...
def main():
    ...
    # find final image size and bin each image into one
    # or several chunk definition.
    ...
    # we need to region of fov to one or more chunks
    ...
    chunks = [[]] * chunk_grid_row * chunk_grid_col
    ...
    for fov in fovs():
        ...
        # check which chunks this fov overlaps
        ...
        # define fov's contributions to each chunk
        for row in range(chunk_row_min, chunk_row_max + 1):
            for col in range(chunk_col_min, chunk_col_max + 1):
                ...
                # where to start copying in the fov
                global_fov_start_x = max(global_fov_start_x, col * chunk_width)
                global_fov_start_y = max(global_start_y, row * chunk_height)

                region = (
                        filename, 
                        (global_start_x, global_end_x, global_start_y, global_end_y), 
                        (global_fov_start_x, global_fov_start_y)
                        )
                ...
                # TODO remove
                region_count = region_count + 1 # TODO remove
    ...
    for row in range(chunk_grid_row):
        for col in range(chunk_grid_col):
            ...
            for region in chunks[row * col]:
                ...
                # TODO check if we can have subdirectories,
                # we would then need to append them
                filepath = IMG_PATH / region[0]
                with BioReader(filepath) as br:
                    (global_start_x, global_end_x, global_start_y, global_end_y) = region[1]
                    (global_fov_start_x, global_fov_start_y) = region[2]
                    ...
                    print(f"copying region from fov : {global_fov_start_x, global_fov_start_y} with width ({region_width}, height {region_height})")

                    br[global_fov_start_y: global_fov_start_y + region_height ,global_fov_start_x:global_fov_start_x + region_width]
